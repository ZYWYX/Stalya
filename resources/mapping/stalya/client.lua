...
Citizen.CreateThread(function()
	...
	while true do
		...
		if veh ~= 0 and GetIsVehicleEngineRunning(veh) or IsPedInAnyPoliceVehicle(ped) then
			hudEnabled = true
			waitTime = 100

		--	if HasPedGotWeapon(ped, shotgun,false) then
		--		RemoveWeaponFromPed(ped, shotgun)
		--	end
			local indicatorType = GetVehicleIndicatorLights(veh)
